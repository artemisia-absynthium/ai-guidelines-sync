...
- **Written** in plan mode at task start, before the first step — then read immediately so it
  ...
  or compaction), and linked as the plan's first line (`Charter: .claude/rules/charter.local.md`)
  so reading or executing the plan forces reading it.
