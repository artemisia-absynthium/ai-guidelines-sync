...
# Colours go through %b; the message goes through %s so a repo-supplied name containing
# escape sequences is printed verbatim, never decoded into terminal control.
info()    { printf '  %b%s%b\n' "$BLUE" "$*" "$NC"; }
success() { printf '  %b✓ %s%b\n' "$GREEN" "$*" "$NC"; }
warn()    { printf '  %b⚠ %s%b\n' "$YELLOW" "$*" "$NC"; }
err()     { printf '%bError: %s%b\n' "$RED" "$*" "$NC" >&2; }
header()  { printf '\n%b%s%b\n' "$BOLD$BLUE" "$*" "$NC"; }
# Every claim of a filesystem change sits in the success branch of that change; a failed
# step is reported here and the caller returns 1 — the run's output never names a change
# that did not happen, and the exit status says the run did not complete.
# Usage: fail_step <verb> <path>
fail_step() { err "Could not $1 $2 — files written before this point are left in place; run: git status"; }
...
# Sidecar names written next to a destination (install_file, merge_guard_hook). Distinctive
# on purpose: the directories are shared with local files, and a plain ".tmp"/".bak" could
# collide. Anything already at a sidecar name — file or symlink — is never overwritten.
TMP_SUFFIX=".ai-guidelines-sync.tmp"
...
migrate_legacy_files() {
    if [ -f ".claude/rules-sync" ] && [ ! -f ".claude/rules-sync.txt" ]; then
        mv ".claude/rules-sync" ".claude/rules-sync.txt" || { fail_step rename ".claude/rules-sync"; return 1; }
    ...
    for skill in "${deprecated_skills[@]}"; do
        if [ -d ".claude/skills/$skill" ]; then
            rm -rf ".claude/skills/$skill" || { fail_step remove ".claude/skills/$skill"; return 1; }
...
write_rules_sync_config() {
    ...
    } > ".claude/rules-sync.txt" || { fail_step write ".claude/rules-sync.txt"; return 1; }
...
cleanup_stale_rules() {
    ...
    for cat_dir in ".claude/rules/synced"/*/; do
        ...
        if [ "$is_active" = false ]; then
            rm -rf "$cat_dir" || { fail_step remove "$cat_dir"; return 1; }
...
write_workflow_file() {
    cat > ".github/workflows/sync-claude-rules.yml" <<WORKFLOW || { fail_step write ".github/workflows/sync-claude-rules.yml"; return 1; }
...
write_skills_manifest() {
    [ "$#" -eq 0 ] && return
    printf '%s\n' "$@" | sort -u > ".claude/skills/.synced-manifest" || { fail_step write ".claude/skills/.synced-manifest"; return 1; }
...
install_file() {
    local src="$1" dest="$2"
    local tmp="$dest$TMP_SUFFIX" bak="$dest$UPSTREAM_BAK_SUFFIX"
    ...
    if [ -L "$tmp" ] || [ -e "$tmp" ] || [ -L "$bak" ] || [ -e "$bak" ]; then
...
# 0 when the input holds exactly one JSON document in the shape merge_guard_hook reads.
...
project_settings_document() {
    local doc
    doc=$(jq -c -s 'if length == 1 then .[0] else {} end' ${1:+"$1"} 2>/dev/null) || doc='{}'
...
# The filesystem gate before the merge: no symlink at the file (a live one would be
# replaced by a regular file, a dangling one followed on creation), no stale sidecar, the
# file created as {} when absent, and a regular file at the end.
ensure_settings_file() {
    local settings_file="$1" stale
    if [ -L "$settings_file" ]; then
        err "$settings_file is a symlink — replace it with a regular file and re-run."
        return 1
    fi
    for stale in "$settings_file$TMP_SUFFIX".*; do
        [ -L "$stale" ] || [ -e "$stale" ] || continue
        err "$stale is left over from an interrupted run — safe to remove; then re-run."
        return 1
    done
    if [ ! -e "$settings_file" ]; then
        # noclobber: the kernel refuses if anything appeared at the name meanwhile.
        ( set -C; echo '{}' > "$settings_file" ) 2>/dev/null || { fail_step create "$settings_file"; return 1; }
    fi
    [ -f "$settings_file" ] || { err "$settings_file is not a regular file — fix it and re-run."; return 1; }
}

# Usage: back_up_settings_json <file>
# Copies the file to <file>.before-priming and prints that path. The backup is the recovery
# path for an uncommitted hand edit; git history is not. Nothing already at the name — an
# earlier backup, a file, a symlink — is ever overwritten or followed.
back_up_settings_json() {
    local backup="$1.before-priming"
    if [ -L "$backup" ] || [ -e "$backup" ]; then
        err "$backup already exists — move it away (it holds an earlier original) and re-run."
        return 1
    fi
    # Exclusive create first so the kernel refuses a name that appeared meanwhile; cp -p
    # then fills it and carries the original's mode.
    if ! ( set -C; : > "$backup" ) 2>/dev/null || ! cp -p "$1" "$backup"; then
        rm -f "$backup"
        fail_step write "$backup"
        return 1
    fi
    printf '%s\n' "$backup"
}
...
merge_guard_hook() {
    ...
    ensure_settings_file "$settings_file" || return 1
    ...
    # The file is read once; the decision and the document acted on are the same bytes.
    local raw
    raw=$(cat "$settings_file") || { err "Could not read $settings_file"; return 1; }
    ...
    if printf '%s\n' "$raw" | settings_json_usable; then
        doc=$raw
    else
        backup=$(back_up_settings_json "$settings_file") || return 1
        doc=$(printf '%s\n' "$raw" | project_settings_document) || { fail_step project "$settings_file"; return 1; }
        primed=true
        warn "$settings_file was not in the shape Claude Code reads and was primed: readable parts kept, the rest dropped. The original is at $backup — review with git diff, then delete the backup before committing; keys from the old shape may remain."
    fi
    [ -n "$doc" ] || { err "$settings_file produced an empty document — nothing written."; return 1; }
    ...
    local outdated
    outdated=$(printf '%s\n' "$doc" | jq "$JQ_SETTINGS_DEFS"'[.hooks.PreToolUse // [] | .[] | select(owned_entry)] | length')

    # A per-run sidecar beside the destination (O_EXCL via mktemp, so nothing planted at
    # the name is followed; same directory, so the final mv is an atomic rename), staged
    # with the interrupt trap so Ctrl-C leaves nothing behind. cp -p carries the original's
    # mode onto the sidecar; the redirect below replaces only its content. jq -e fails on an
    # empty result, so an empty document can never be renamed over the file.
    local tmp
    tmp=$(mktemp "$settings_file$TMP_SUFFIX.XXXXXX") || { fail_step create "$settings_file$TMP_SUFFIX"; return 1; }
    UPSTREAM_STAGE="$tmp"
...
setup_project() {
    ...
    setup_directories || { fail_step create ".claude/ or .github/workflows/"; return 1; }
